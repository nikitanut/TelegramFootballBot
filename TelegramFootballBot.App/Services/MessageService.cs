...
public class MessageService(IBotClient botClient, ILogger logger) : IMessageService
{
    public async Task<List<SendMessageResponse>> SendMessagesAsync(string text, IEnumerable<ChatId> chats, ReplyMarkup? replyMarkup = null)
    {
    ...
    private async Task<Message> EditMessageAsync(Message message, string text)
    {
        ...
        return await botClient.EditMessageTextAsync(message.Chat.Id, message.MessageId, text, cancellationToken: cts.Token);
    }

    public async Task<Message> SendMessageToBotOwnerAsync(string text, ReplyMarkup? replyMarkup = null)
    {
        if (AppSettings.NotifyOwner)
        {
            ...
            {
                logger.Error("An error occured while sending a message to the bot owner", ex);
            }
        }
    ...
    public async Task<Message> SendMessageAsync(ChatId chatId, string text, ReplyMarkup? replyMarkup = null)
    {
        using var cts = new CancellationTokenSource(Constants.ASYNC_OPERATION_TIMEOUT);
        return await botClient.SendTextMessageAsync(chatId, text, replyMarkup: replyMarkup, cancellationToken: cts.Token);
    }
    ...
    public async Task DeleteMessageAsync(ChatId chatId, int messageId)
    {
        ...
        {
            using var cts = new CancellationTokenSource(Constants.ASYNC_OPERATION_TIMEOUT);
            await botClient.DeleteMessageAsync(chatId, messageId, cancellationToken: cts.Token);
        }
        catch (Exception ex)
        {
            logger.Error(ex, "An error occurred while deleting message");
        }
    }
    ...
    {
        using var cts = new CancellationTokenSource(Constants.ASYNC_OPERATION_TIMEOUT);
        await botClient.EditMessageReplyMarkupAsync(chatId, messageId, replyMarkup: new[] { Array.Empty<InlineKeyboardButton>() }, cancellationToken: cts.Token);
    }
}
